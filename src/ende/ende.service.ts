...
export class EndeService {
  ...
  static encodeToDatabase(password: string): string {
    ...
    const hash = pbkdf2.pbkdf2Sync(password, salt, iteration, 32, 'sha256').toString('base64');
  ...
  static verify(password: string, hash: string): boolean {
    ...
    const salt = parts[2];
    const password_hash = pbkdf2.pbkdf2Sync(password, salt, iteration, 32, 'sha256').toString('base64');
